...
**seo-agi-verify** is the "someone." It reads every tag, searches for the real source, and either confirms the claim, corrects it, or flags it for a human.
...
Each stage is independent. You can use seo-agi-verify on any HTML/Markdown file that contains verification tags, even if it wasn't generated by SEO-AGI.
...
```bash
git clone https://github.com/gbessoni/seo-agi-verify.git ~/.codex/skills/seo-agi-verify
```

Or for Claude Code:

```bash
git clone https://github.com/gbessoni/seo-agi-verify.git ~/.claude/skills/seo-agi-verify
```
...
```bash
python3 ~/.codex/skills/seo-agi-verify/scripts/verify.py summary ~/Documents/SEO-AGI/pages/
```
...
```
/seo-agi-verify ~/Documents/SEO-AGI/pages/fll-parking.html
```
...
```
/seo-agi-verify ~/Documents/SEO-AGI/pages/*.html
```
